...
async function getListOfFileWithTypeInDirWithFilter(dirPath, fileType, filter) {
    let files = [];
    await Walk.walk(dirPath, async (err, pathname, dirent) => {
        ...
        if (dirent.isFile() && path.extname(dirent.name) === fileType && filter.includes(dirent.name)) {
...
module.exports = {
    ...
    getListOfFileWithTypeInDirWithFilter,
};
