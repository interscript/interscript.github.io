...
function getSortedUniqueValues<T, K extends keyof T>(
  array: T[],
  key: K
): T[K][] {
  const arr: T[K][] = Array.from(new Set(array.map((i) => i[key]))).filter(
    (v) => v !== undefined
  );
  arr.sort();
  return arr;
...
export const SystemSelector: React.FC<{
  ...
  const [systemSpec, updateSystemSpec] = useState<
  ...
  const systemIDs = getSortedUniqueValues(
    supportedSystems.filter(
      (ss) =>
        getSortedUniqueValues(availableSystems, "authority").indexOf(
          ss.authority
        ) >= 0
    ),
    "id"
  ...
  function autoSelectAll(availableSystems: ScriptConversionSystem[]) {
    ...
    if (
      systemSpec.authority !== undefined &&
      getSortedUniqueValues(availableSystems, "id").length === 1
    ) {
        ...
        <SystemPropertyChoiceList>
          {sourceSystemCodes.map((sourceSystemCode) => (
            <Choice
              ...
              available={
                availableSystems.filter((s) => s.source === sourceSystemCode)
                  .length > 0
              }
        ...
        <SystemPropertyChoiceList>
          {langCodes.map((langCode) => (
            <Choice
              ...
              key={langCode}
              available={
                availableSystems.filter((s) => s.lang === langCode).length > 0
              }
              selected={systemSpec.lang === langCode}
              onForce={() => updateSystemSpec({ lang: langCode })}
              onSelect={() =>
                updateSystemSpec((spec) => ({ ...spec, lang: langCode }))
              }
            >
              <Lang code={langCode} />
            </Choice>
        ...
        <SystemPropertyChoiceList>
          {targetSystemCodes.map((targetSystemCode) => (
            <Choice
              ...
              available={
                availableSystems.filter((s) => s.target === targetSystemCode)
                  .length > 0
              }
        ...
        <SystemPropertyChoiceList>
          {authorityCodes.map((authorityCode) => (
            <Choice
              ...
              key={authorityCode}
              available={
                availableSystems.filter((s) => s.authority === authorityCode)
                  .length > 0
              }
        ...
        <SystemPropertyChoiceList>
          {systemIDs.map((id) => (
            <Choice
              ...
              onForce={() =>
                updateSystemSpec({ authority: systemSpec.authority, id })
              }
...
const WritingSystem: React.FC<{ code: WritingSystemCode }> = function ({
  code,
}) {
