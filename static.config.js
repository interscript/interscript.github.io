...
const saveHtmls = (htmlData, path) => {
    const promises = htmlData.map((html) => {
        ...
        return new Promise(resolve => {
            return fs.writeFile(`${path}/${name}.html`, html.data,() =>  resolve(staticPage))
        });
    });
    return Promise.all(promises);
}

const scanAsciiDocs = async (path) => {
...
async function loadBlog() {
    return scanAsciiDocs('./blog').then((files) => {
        const capitalize = (s) => {
            return s && s[0].toUpperCase() + s.slice(1);
        }
        const htmls = files.map((file) => {
            const fullname = file.split('/')[file.split('/').length - 1].replace('.adoc', '');
            const date = fullname.substring(0,10);
            const name = fullname.substring(11);
            const label = capitalize(name).replace(/-/g, ' ');

            const buffer = fs.readFileSync(file);
            return { name: name, date: date, data: asciidoctor.convert(buffer), label: label };
        });
        // TODO: get metadata
        // push htmls to public directory
        return saveHtmls(htmls, './public/blog');
    })
}
...
export default {
    ...
    getRoutes: async () => {
        ...
        // load docs
        docsList = await loadDocs();
        blogList = await loadBlog();
                ...
                children: (blogList || []).map((blogPost) => {
                    return {
                        ...
                        getData: () => ({
                            blogList,
                            blogPost,
                            html: fs.readFileSync(blogPost.template, 'utf8')
                        })
                    }
                })
            },
